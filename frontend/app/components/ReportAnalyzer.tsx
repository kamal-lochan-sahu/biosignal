...
function generateExplanation(
  vitals: VitalsData,
  result: { risk_score: number; risk_level: string; top_factors: { feature: string; impact: number }[] }
): string {
  ...
  if (level === 'HIGH') {
    ...
    return `✅ LOW RISK — ${pct}% chance of deterioration in next 6 hours.\n\n${issueText}\n\nContinue routine monitoring as scheduled. The patient appears stable. Reassess if any symptoms change.`
  }
}
...
export default function ReportAnalyzer() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setError('Analysis failed — make sure backend is running on port 8000')
    }
    setLoading(false)
  }
    ...
    <div className="space-y-4">
      ...
      <button onClick={handleAnalyze} disabled={loading}
        ...
        {loading
          ...
          : <><Activity className="w-4 h-4" />Analyze Patient Risk</>}
      </button>
        ...
        <div className="space-y-4">
          <div className={`border rounded-xl p-5 ${riskBg}`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider">ML Risk — {vitals.name}</p>
                <p className={`text-5xl font-black mt-1 ${riskColor}`}>{Math.round(result.risk_score * 100)}%</p>
                <p className={`text-sm font-bold mt-1 ${riskColor}`}>{result.risk_level} RISK — Next 6 Hours</p>
              </div>
            ...
            <div className="space-y-2">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Top Risk Factors</p>
              {result.top_factors.map((f, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-black/30 rounded-full h-1.5">
                    <div className={`h-1.5 rounded-full ${f.impact > 0 ? 'bg-red-400' : 'bg-green-400'}`}
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-gray-500 ml-auto">{vitals.name}, {vitals.age}y</span>
            </div>
